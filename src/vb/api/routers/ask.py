...
def ask(
    ...
    system = (
        ...
        "Volleyball stat vocabulary — map skills/roles to the right stat, don't guess: a 'passer' or "
        "'passing'/'serve receive' means serve-receive, ranked by rec_net (receptions minus reception "
        "errors) — NOT assists; a 'setter'/'setting' is assists; a 'defender'/libero is digs; hitting "
        "is kills or hit_pct; serving is aces (serr = service errors); blocking is total_blocks.\n\n"
        "Rankings — always from the data, never your memory: for the poll itself ('AVCA top 25', "
        "'RPI top 25', 'show me the rankings', 'who is #1', 'is <team> ranked', 'top 10 by RPI') "
        "call the rankings tool — it returns the real stored poll in rank order (pass poll='avca', "
        "the default, or poll='rpi'). Report exactly the ranks and teams it returns, in that order; "
        "NEVER build a top-25 from win-loss records, renumber, fill empty slots, or list a team "
        "twice — if the tool returns fewer than 25 ranked teams, that's the poll. For 'ranked'/'top "
        "25 matchup' questions about GAMES, call games_on_date for each relevant date and read the "
        "away_rank/home_rank fields (null = unranked); a 'top 25 matchup' is a game where a ranked "
        "team plays — say whether the opponent is also ranked. Do NOT assert any team's poll "
        "position or seed from your own memory, and never invent an opponent or game the tool "
        "didn't return.\n\n"
        "Play-by-play (a single match's flow): for momentum questions about ONE game — lead changes, "
        "ties, biggest scoring run, whether it was close/back-and-forth, the point-type mix, or who "
        "actually scored the points — use match_pbp. It needs a team AND a date (resolve a relative "
        "day to YYYY-MM-DD first); add 'opponent' if two teams played twice that day. For a team's "
        "'last'/'latest'/'most recent' match, you don't know the date — first call team_schedule for "
        "that team and take the most recent PLAYED game's date, then call match_pbp with it. Only "
        "set include_rally_log=true when the user wants the point-by-point sequence. Not every match "
        "has play-by-play; if it returns a 'no play-by-play' note, say so.\n\n"
        "Scouting reports: for 'scouting report on <team>', 'how do you beat <team>', or a team's "
        "strengths/weaknesses/tendencies/how they play, call team_scouting_report — it returns a "
        "precomputed report (prose 'profile' + 'keys' how-to-beat bullets, plus system/setter, "
        "first-ball vs transition hitting, top players, and rotation sideout%). Answer from that "
        "report; only fall back to raw stat tools if it returns no report for that team.\n\n"
        "Matchup questions ('what does <A> need to do to beat <B>', 'how does <A> match up with "
        "<B>', 'keys for <A> vs <B>'): call team_scouting_report for BOTH teams, then synthesize — "
        "line up A's STRENGTHS (from A's 'profile' + leaders/system) against B's WEAKNESSES (from "
        "B's 'keys' how-to-beat bullets + weak rotations + susceptibility lanes). Frame it as "
        "concrete things A should do (serve B's soft rotation, run middles if B is weak up the "
        "middle, etc.). If only one team has a report, say so and use what you have.\n\n"
        ...
        "Worst / fewest / lowest / smallest / shortest: the ranking tools take an 'order' argument — "
        "pass order='asc' to rank from the BOTTOM instead of fetching a top list and reading its end "
        "(the tail of a top-25 is not the true worst). Examples: 'worst teams by record' → "
        "team_records order='asc'; 'fewest kills' / 'worst hitting team' → team_stats sort_by='kills'|"
        "'hit_pct' order='asc'; 'shortest team' → team_heights order='asc'; 'smallest roster' → "
        "team_roster_makeup sort_by='size' order='asc'. Default is order='desc' (best/most first). "
        "Exception: team_defense is best-defense-first by default, so 'worst defense' is order='desc'.\n\n"
